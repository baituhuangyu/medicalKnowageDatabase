...
class PrepareTagData(object):
    def __init__(self, conf, mode="train"):
        ...
        :param mode:
        """
        self.dataPath = os.path.join(os.path.dirname(__file__), os.pardir)
        self.config = conf
        self.mode = mode
    ...
    def __load_chinese_vocab(self):
        cv = dict()
        with codecs.open(os.path.join(self.dataPath, "data/all_data/chinese_vocab.txt"), "r", "utf8") as f:
    ...
    def __read_dataset(self):
        if self.mode == "train":
            dataset_path = os.path.join(self.dataPath, "data/all_data/all_data_tagged.txt")
        elif self.mode == "test":
            dataset_path = os.path.join(self.dataPath, "data/all_data/all_data_tagged.txt")
        else:
